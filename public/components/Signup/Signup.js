function Signup() {
    ...
    form.addEventListener('submit', (e) => {
        ...
        // Позже: fetch('/api/signup', { method: 'POST', body: JSON.stringify({ email, password }) })
        alert('Регистрация выполнена (тест)');
    });

    // Обработчики для toggle-password
    ...
    const eyeIcons = toggleButtons;
    toggleButtons.forEach((btn, index) => {
        btn.addEventListener('click', () => {
            const input = passwordInputs[index];
            const icon = eyeIcons[index].querySelector('svg');
