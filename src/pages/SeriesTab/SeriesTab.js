...
import { fetchGenres, fetchMoviesByGenreId } from '@shared/api/genresApi'; // Assuming fetchSeriesByGenreId exists or adapt
...
class SeriesTab {
    ...
    async render() {
        ...
        await this.#updateSeriesByGenres();
    ...
    setupPlayButton() {
        ...
        if (playButton && seriesContainer) {
            playButton.addEventListener('click', () => {
                seriesContainer.scrollIntoView({
                    behavior: 'smooth',
                    block: 'start',
                });
            });
    ...
    async #setupYearDropdownContent() {
        ...
        const years = [...new Set(this.#currentSeries.map(s => new Date(s.release_date).getFullYear()))]
            .sort((a, b) => b - a);
    ...
    async #updateSeriesByGenres() {
        ...
        if (selectedIds.length === 0) {
            ...
            series = resp.movies.map(s => this.#mapSeries(s)); // Assuming 'movies' is the key for series
        } else if (selectedIds.length === 1) {
            // Assuming fetchSeriesByGenreId exists; if not, fetch all and filter client-side
            const resp = await fetchMoviesByGenreId(selectedIds[0]); // Adapt if needed
            series = resp.movies.map(s => this.#mapSeries(s));
            const genre = this.#allGenres.find(g => g.id == selectedIds[0]);
            sectionTitle.textContent = genre?.name || 'Unknown';
        } else {
            const responses = await Promise.all(selectedIds.map(id => fetchMoviesByGenreId(id))); // Adapt if needed
            ...
            const map = new Map();
            responses.forEach(r => r.movies.forEach(m => map.set(m.media_id, this.#mapSeries(m))));
            series = [...common].map(id => map.get(id));
    ...
    #mapSeries(series) {
        return {
            ...
            type: 'series',
            user_rating: series.user_rating || { likes: 0 }
        };
    }

    renderCurrentSeries() {
        const container = this.#parent.querySelector('#seriesContainer');
        container.innerHTML = '';

        let list = this.#applyFiltersAndSort([...this.#currentSeries]);

        list.forEach(series => {
            const el = document.createElement('div');
            container.appendChild(el);
            new FilmCard(el, this.#app).render(series);
        });
    }

    #applyFiltersAndSort(seriesList) {
        let result = seriesList;

        if (this.#selectedYear !== null) {
            result = result.filter(s => parseInt(s.release_date) === this.#selectedYear);
        }
